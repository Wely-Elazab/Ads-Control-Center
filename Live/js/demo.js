...
// الأرقام في المشهد التاني بتعدّ لحد قيمتها (data-count، وdata-money = بالجنيه) بصيغة اللغة الحالية.
//  - «تقليل الحركة» في الجهاز: المشهد التالت (التنبيهات) ثابت من غير حركة
...
(function () {
  ...
  var current = 0, timer = null, visible = true, countRaf = null, countDone = null;
  ...
  function fmt(n, money) {
    ...
    if (isAr()) {
      ...
      return money ? s + ' ج.م' : s;
    }
    return money ? 'EGP ' + s : s;
  }
  ...
  function schedule() {
    ...
    timer = setTimeout(function () { show((current + 1) % scenes.length); schedule(); }, DURATIONS[current]);
  }
  ...
  // يبدأ من الأول كل ما يرجع يظهر — عشان الزائر يشوف القصة كاملة
  function resume() { if (!visible || document.hidden) return; show(current); schedule(); }
  ...
  document.addEventListener('visibilitychange', function () { if (document.hidden) clearTimeout(timer); else resume(); });
